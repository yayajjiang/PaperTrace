...
export default function Page() {
  const { t } = useLang();
  const html = `<h1>OLMo-2 白盒演进与日志剖析</h1>
<blockquote>
...
<hr>
<h2 id="1-sjdj-zmwqkfkydd-competitive-xn">1 设计动机: 证明完全开放可以达到 competitive 性能</h2>
...
<blockquote>
<p>译者注: 表 1 的数据非常有力地回答了 OLMo 2 的核心问题. OLMo 2 7B 的 MMLU 63.7 已经接近 Llama 3.1 8B 的 66.5 和 Qwen 2.5 7B 的 65.0. 这意味着 OLMo 2 首次在「完全开放」的约束下, 在性能上达到了「开放权重」模型的主流水平. 论文中的一个诚实披露是: 「we could not estimate compute for any Mistral model because their total training token count is unknown」——这恰恰突显了完全开放模型的独特价值.</p>
</blockquote>
<hr>
<h2 id="2-hxjgyj-cfcsh-ln-dwdxgc">2 核心架构演进: 从非参数化 LN 到稳定性工程</h2>
...
<p>译者注: 输出 norm(Post-LN)与输入 norm(Pre-LN)的对比是一个经典的训练稳定性话题. Pre-LN 的直觉是: 在进入注意力/MLP 之前先归一化, 可以限制输入的数值范围. Post-LN 的直觉是: 在注意力/MLP 的输出上归一化, 可以更直接地控制残差流的尺度. 单独使用 Post-LN 可能导致训练早期的不稳定, 但结合 QK-Norm 后, query 和 key 的归一化确保了注意力分数的尺度有界, 从而补偿了 Post-LN 的初期不稳定性. 论文明确指出单独应用输出 RMSNorm 或 QK-Norm 都不会产生良好结果, 但一起应用时 gradient spike score 从 0.108 降至 0.069.</p>
</blockquote>
...
<blockquote>
<p>译者注: OLMo 2 在 Z-Loss 的实现上踩了一个重要的坑: Flash Attention 库提供的 fused Z-Loss 实现与纯 PyTorch 实现在 backward pass 中行为不同. 虽然 forward pass 产生相同的结果, 但 backward pass 中的数值精度差异导致训练曲线在数十万步后逐渐分叉. 这个发现极具工程价值——它提醒所有使用 Flash Attention 的团队: fused kernel 的 backward 行为需要独立验证, 不能假设与 reference 实现等价.</p>
</blockquote>
<hr>
<h2 id="3-gjcx-xlwdxgcdbxcs">3 关键创新: 训练稳定性工程的八项措施</h2>
...
<blockquote>
<p>译者注: 重复 n-gram 导致 loss spikes 的机制值得分析. 当模型看到大量重复的 token 序列时, 自回归目标(next token prediction)变得「过于简单」——模型可以几乎确定性地预测下一个 token. 这导致梯度更新方向高度一致, 可能引发优化器状态的共振效应. 但论文也坦诚地指出, 这种关系不是确定性的: 相同的 n-gram 对不同规模模型,不同数据顺序的影响不同.</p>
</blockquote>
...
<blockquote>
<p>译者注: 这个发现 contradicts 机器学习的民间智慧如「更高学习率总是更好」. 它表明在中等规模上(几 T token), 学习率选择对最终性能的影响比预期小——不同学习率的变体在退火后性能差异不到 2 个百分点. 这一发现的实际意义是: 团队不需要在学习率上进行昂贵的 grid search, 而是可以将资源投入到数据质量和退火配方上.</p>
</blockquote>
<h3 id="4-2-dolmino-sjkc">4.2 Dolmino: 数据课程</h3>
<p>Mid-training 的数据混合 Dolmino 包括: DCLM 基线过滤网页(~50%),Stack Exchange Q&amp;A,FLAN(去污染),Wiki,arXiv,peS2o, 以及数学数据(OpenWebMath,GSM8K,TuluMath,DolminoSynthMath).</p>
<blockquote>
<p>译者注: Dolmino 的组成反映了 mid-training 的核心目标: 不是简单地「继续训练」, 而是「有针对性地修补能力缺陷」. 消融实验显示, 仅添加 FineWeb-Edu&gt;=2 过滤的网页数据就能提升 MMLU 1.3 点; 再加入数学和指令数据后, GSM8K 从 30.1 提升到 47.8——这是一个 58% 的相对提升, 而额外成本仅为 50B token(约 1.2% 的总训练量).</p>
</blockquote>
<h3 id="4-3-micro-annealing-sjgcdkxff">4.3 Micro-Annealing: 数据工程的科学方法</h3>
...
<blockquote>
<p>译者注: Micro-annealing 的方法论价值在于将数据质量评估的成本降低了 10 倍以上. 传统的数据工程需要完整的训练运行(数 T token, 数周时间)来验证一个数据源的效果; micro-annealing 允许在几天内评估数十个候选数据源. 发现的「重复有益」现象也很有趣: 它表明在退火阶段, 数据的「重复暴露」可以加深模型对特定领域的理解, 而不是像预训练阶段那样导致过拟合.</p>
</blockquote>
<h3 id="4-4-mxrh-souping">4.4 模型融合 (Souping)</h3>
...
<blockquote>
<p>译者注: 模型融合(model soup)在 CV 领域已被证明有效, 但在 LM 预训练中的应用相对较少. OLMo 2 的贡献在于系统地验证了 souping 在 mid-training 阶段的稳健性. 其背后的直觉是: 不同数据顺序导致模型收敛到损失景观中的不同局部最小值, 这些最小值的简单平均往往位于一个更平坦,更泛化的区域.</p>
</blockquote>
<hr>
<h2 id="5-hxl-tulu-3-rlvr">5 后训练: Tulu 3 + RLVR</h2>
...
<blockquote>
<p>译者注: OLMo 2 的后训练有几个值得注意的点. 第一, 超参数调整发现 OLMo 2 需要「显著更高的学习率」相比 Llama 3.1——7B SFT 最佳 <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mn>2</mn><mo>×</mo><msup><mn>10</mn><mrow><mo>−</mo><mn>5</mn></mrow></msup></mrow><annotation encoding="application/x-tex">2 \\times 10^{-5}</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.7278em;vertical-align:-0.0833em;"></span><span class="mord">2</span><span class="mspace" style="margin-right:0.2222em;"></span><span class="mbin">×</span><span class="mspace" style="margin-right:0.2222em;"></span></span><span class="base"><span class="strut" style="height:0.8141em;"></span><span class="mord">1</span><span class="mord"><span class="mord">0</span><span class="msupsub"><span class="vlist-t"><span class="vlist-r"><span class="vlist" style="height:0.8141em;"><span style="top:-3.063em;margin-right:0.05em;"><span class="pstrut" style="height:2.7em;"></span><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mtight">−</span><span class="mord mtight">5</span></span></span></span></span></span></span></span></span></span></span></span> vs Llama 3.1 的约 <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mn>1</mn><mo>×</mo><msup><mn>10</mn><mrow><mo>−</mo><mn>5</mn></mrow></msup></mrow><annotation encoding="application/x-tex">1 \\times 10^{-5}</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.7278em;vertical-align:-0.0833em;"></span><span class="mord">1</span><span class="mspace" style="margin-right:0.2222em;"></span><span class="mbin">×</span><span class="mspace" style="margin-right:0.2222em;"></span></span><span class="base"><span class="strut" style="height:0.8141em;"></span><span class="mord">1</span><span class="mord"><span class="mord">0</span><span class="msupsub"><span class="vlist-t"><span class="vlist-r"><span class="vlist" style="height:0.8141em;"><span style="top:-3.063em;margin-right:0.05em;"><span class="pstrut" style="height:2.7em;"></span><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mtight">−</span><span class="mord mtight">5</span></span></span></span></span></span></span></span></span></span></span></span>. 这说明不同架构/初始化对后训练超参数有实质性影响, 不能简单复用其他模型的配方. 第二, RLVR 的多阶段迭代策略(先在混合数据上, 再在 GSM8K 上, 最后在 MATH 上)解决了单一 RLVR 运行可能 overfit 到某些任务的问题.</p>
</blockquote>
<hr>
<h2 id="6-hxdb-wqkfmxdxngd">6 横向对比: 完全开放模型的性能拐点</h2>
<table>
<thead>
<tr>
...
<blockquote>
<p>表 5: OLMo 2 Instruct 与专有模型性能对比.</p>
</blockquote>
<p>OLMo 2 13B Instruct 达到 63.5 平均分, 超越 Llama 3.1 8B Instruct(59.1)和 Tulu 3 8B(60.7), 接近 Qwen 2.5 7B(61.6). 这意味着 OLMo 2 在「完全开放」的约束下, 首次在性能上达到了「开放权重」模型的主流水平.</p>
<hr>
<h2 id="7-jxxyfx">7 局限性与风险</h2>
<h3 id="7-1-xlgmxz">7.1 训练规模限制</h3>
<p>OLMo 2 7B 仅训练 4.05T token, 而 Llama 3.1 8B 训练了 15T+ token. 这反映了 AI2 作为非营利机构的计算预算限制. 在更大规模上, 完全开放模型与商业模型之间的差距可能再次扩大.</p>
<h3 id="7-2-dyynlbz">7.2 多语言能力不足</h3>
<p>OLMo 2 主要针对英语优化, 多语言能力弱于 Qwen 2.5 等模型. 这在日益全球化的 AI 应用中是一个明确的短板.</p>
<h3 id="7-3-32b-mxd-gqa-byzx">7.3 32B 模型的 GQA 不一致性</h3>
<p>32B 模型使用 GQA 而 7B/13B 使用 MHA, 这种不一致性增加了推理复杂度. 对于需要在同一推理框架中服务多个模型尺寸的应用, 这种不一致性带来了额外的工程负担.</p>
<hr>
<h2 id="8-lsdw">8 历史定位</h2>
<p>OLMo 2 标志着完全开放语言模型的一个重要转折点. 在它之前, 完全开放模型(OLMo 1,Pythia,Amber)的性能明显落后于开放权重模型; OLMo 2 首次证明, 在保持完全开放(权重,代码,数据,Checkpoint,日志)的同时, 可以达到 competitive 的性能. 这一成就的影响是深远的: 它为研究机构争取开放政策支持提供了实证, 为开源社区提供了可复现的 SOTA 级训练配方.</p>
<p>OLMo 2 的稳定性工程方法论(8 项措施的组合)和数据工程工具(micro-annealing,souping)已经被其他团队广泛引用和采用, 包括 Hugging Face 的 SmolLM 系列. 从技术演进看, OLMo 2 是 OLMo 1 的「性能追赶」版本, 而 OLMo 3 则转向架构创新(混合线性注意力), 反映了 AI2 从「证明可行性」到「追求创新」的战略转变.</p>
<hr>
<h2 id="9-zj">9 总结</h2>
...
`;
  const toc: { level: number; id: string; text: string }[] = [{"level":2,"id":"1-sjdj-zmwqkfkydd-competitive-xn","text":"1 设计动机: 证明完全开放可以达到 competitive 性能"},{"level":2,"id":"2-hxjgyj-cfcsh-ln-dwdxgc","text":"2 核心架构演进: 从非参数化 LN 到稳定性工程"},{"level":3,"id":"2-1-sc-rms-norm-qk-norm-dzhxy","text":"2.1 输出 RMSNorm + QK-Norm 的组合效应"},{"level":3,"id":"2-2-z-loss-dgcxj","text":"2.2 Z-Loss 的工程陷阱"},{"level":2,"id":"3-gjcx-xlwdxgcdbxcs","text":"3 关键创新: 训练稳定性工程的八项措施"},{"level":3,"id":"3-1-sjgl-zf-n-gram","text":"3.1 数据过滤: 重复 n-gram"},{"level":3,"id":"3-2-cshgj","text":"3.2 初始化改进"},{"level":3,"id":"3-3-ccstz","text":"3.3 超参数调整"},{"level":2,"id":"4-ljdxlythffl","text":"4 两阶段训练与退火方法论"},{"level":3,"id":"4-1-xxs-plateau-xx","text":"4.1 学习率 Plateau 现象"},{"level":3,"id":"4-2-dolmino-sjkc","text":"4.2 Dolmino: 数据课程"},{"level":3,"id":"4-3-micro-annealing-sjgcdkxff","text":"4.3 Micro-Annealing: 数据工程的科学方法"},{"level":3,"id":"4-4-mxrh-souping","text":"4.4 模型融合 (Souping)"},{"level":2,"id":"5-hxl-tulu-3-rlvr","text":"5 后训练: Tulu 3 + RLVR"},{"level":2,"id":"6-hxdb-wqkfmxdxngd","text":"6 横向对比: 完全开放模型的性能拐点"},{"level":2,"id":"7-jxxyfx","text":"7 局限性与风险"},{"level":3,"id":"7-1-xlgmxz","text":"7.1 训练规模限制"},{"level":3,"id":"7-2-dyynlbz","text":"7.2 多语言能力不足"},{"level":3,"id":"7-3-32b-mxd-gqa-byzx","text":"7.3 32B 模型的 GQA 不一致性"},{"level":2,"id":"8-lsdw","text":"8 历史定位"},{"level":2,"id":"9-zj","text":"9 总结"}];
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      <LlmGuideNav currentRoute="14-models/14.4-olmo/02-olmo-2/02-olmo-2-bhyjyrzpx" />
      <main className="flex-1 min-w-0 px-6 pt-20 pb-12 lg:pt-12">
        <LlmGuideChapterBar currentRoute="14-models/14.4-olmo/02-olmo-2/02-olmo-2-bhyjyrzpx" />
        <header className="mb-12">
          <h1 className="text-3xl font-bold tracking-tight mb-2">OLMo-2 白盒演进与日志剖析</h1>
