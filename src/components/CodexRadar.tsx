...
type CommunitySignal = { id: string; title: string; url: string; primaryUrl?: string | null; platform: string; metrics: Record<string, number>; officialEvidence?: boolean; evidence: string; capturedAt?: string };
...
export function CodexRadar() {
  ...
  return <div className="grid gap-5 lg:grid-cols-2">
    ...
    <section className="rounded-2xl border border-paper-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="mt-4 space-y-3">{verified.map((item) => <article key={item.id} className="rounded-xl border border-emerald-200 p-3 text-sm dark:border-emerald-900"><a href={item.url} target="_blank" rel="noopener noreferrer" className="block hover:text-emerald-700"><b className="block dark:text-white">{item.title}</b><span className="mt-1 block text-xs font-bold text-emerald-700">{t("Official source linked", "已回链官方来源")} · {item.platform}</span></a><div className="mt-2 flex items-center gap-3 text-[11px]"><a href={item.primaryUrl || item.url} target="_blank" rel="noopener noreferrer" className="font-bold text-emerald-700">{t("Open official evidence", "查看官方证据")} ↗</a>{item.capturedAt && <span className="text-paper-800/45 dark:text-slate-500">{t("captured", "采集")} {new Date(item.capturedAt).toLocaleDateString(lang === "zh" ? "zh-CN" : "en-US")}</span>}</div></article>)}{leads.map((item) => <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer" className="block rounded-xl border border-paper-100 p-3 text-sm dark:border-slate-800"><b className="block dark:text-white">{item.title}</b><span className="mt-1 block text-xs text-paper-800/50 dark:text-slate-400">{t("Community observation — official source not linked", "社区观察：尚未回链官方来源")} · {item.platform}{item.capturedAt ? ` · ${t("captured", "采集")} ${new Date(item.capturedAt).toLocaleDateString(lang === "zh" ? "zh-CN" : "en-US")}` : ""}</span></a>)}{community.length === 0 && <p className="text-sm text-paper-800/55 dark:text-slate-400">{t("No public Codex discussion is in the current snapshot.", "当前快照没有公开 Codex 讨论。")}</p>}</div>
      <p className="mt-4 text-xs leading-relaxed text-paper-800/50 dark:text-slate-400">{t("Community engagement stays platform-local and is neither a benchmark nor a product-performance claim. Only direct OpenAI or openai/codex links receive the verified label.", "社区互动只在平台内解释，不是基准或产品性能主张；只有直达 OpenAI 或 openai/codex 的链接才会获得已核验标记。")}</p>
    </section>
  </div>;
}
